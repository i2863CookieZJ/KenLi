package com.dylan.uiparts.keywordflow;
...
public class KeywordsFlow extends FrameLayout implements OnGlobalLayoutListener {
	...
	public int getType2TextSize() {
		return type2TextSize;
	}

	public void setType2TextSize(int type2TextSize) {
		this.type2TextSize = type2TextSize;
	}
	...
	public boolean feedKeywordAll(ArrayList<String> keyword) {
		boolean result = false;
	...
	private boolean show() {
		if(showType == 1) {
			...
			if(width > 0 && height > 0 && vecKeywords != null
				...
				// 关键字的个数
				int size = vecKeywords.size();
				int[][] xy = new int[size][4];
				int[] strWidth = new int[size];
				int strHeight = 60;
				for(int i = 0; i < size; i++) {
					String keyword = vecKeywords.get(i);
					// 实例化TextView
					...
					txt.setTextSize(TypedValue.COMPLEX_UNIT_SP, type2TextSize);
					txt.setPadding(10, 10, 10, 10);
					txt.setBackgroundDrawable(getResources().getDrawable(R.drawable.keywordflow_word_border));
					txt.setGravity(Gravity.CENTER);

					// 获取文本长度
					Paint paint = txt.getPaint();
					try {
						strWidth[i] = (int) Math.ceil(paint
								.measureText(keyword)) + 30;
					} catch (Exception e) {
						e.printStackTrace();
					}
					...
					if(i == 0) {
						...
						try {
							...
							strHeight = (int) Math.ceil(fm.descent - fm.ascent) + 30;
						} catch (Exception e) {
							e.printStackTrace();
						}
					} else {
						if((xy[i - 1][0] + strWidth[i - 1] + strWidth[i]) > width) {
							xy[i][0] = 0;
							xy[i][1] = xy[i - 1][1] + strHeight;
							xy[i][2] = 0;
							xy[i][3] = 0;
						} else {
							xy[i][0] = xy[i - 1][0] + strWidth[i - 1];
							xy[i][1] = xy[i - 1][1];
							xy[i][2] = 0;
							xy[i][3] = 0;
						}
					}
					layParams.leftMargin = xy[i][IDX_X];
					layParams.topMargin = xy[i][IDX_Y];
					addView(txt, layParams);
					// 动画
					AnimationSet animSet = getAnimationSet(xy[i], 0, 0,
							txtAnimInType);
					txt.startAnimation(animSet);
				}
				return true;
