 ...
 *******************************************************************************/
package com.higgses.griffin.imageloader.core.decode;
...
public class BaseImageDecoder implements ImageDecoder {
	...
	public Bitmap decode(ImageDecodingInfo decodingInfo) throws IOException {
		...
		try {
			...
			GinULo.closeSilently(imageStream);
		}

		if (decodedBitmap == null) {
			GinULog.e(ERROR_CANT_DECODE_IMAGE, decodingInfo.getImageKey());
		} else {
	...
	protected ExifInfo defineExifOrientation(String imageUri) {
		...
		try {
			...
			GinULog.w("Can't read EXIF tags from file [%s]", imageUri);
		}
	...
	protected Options prepareDecodingOptions(ImageSize imageSize, ImageDecodingInfo decodingInfo) {
		...
		if (scaleType == ImageScaleType.NONE) {
			...
			scale = GinUImageSize.computeMinImageSampleSize(imageSize);
		} else {
			ImageSize targetSize = decodingInfo.getTargetSize();
			boolean powerOf2 = scaleType == ImageScaleType.IN_SAMPLE_POWER_OF_2;
			scale = GinUImageSize.computeImageSampleSize(imageSize, targetSize, decodingInfo.getViewScaleType(), powerOf2);
		}
		if (scale > 1 && loggingEnabled) {
			GinULog.d(LOG_SUBSAMPLE_IMAGE, imageSize, imageSize.scaleDown(scale), scale, decodingInfo.getImageKey());
		}
	...
	protected Bitmap considerExactScaleAndOrientatiton(Bitmap subsampledBitmap, ImageDecodingInfo decodingInfo,
		...
		if (scaleType == ImageScaleType.EXACTLY || scaleType == ImageScaleType.EXACTLY_STRETCHED) {
			...
			float scale = GinUImageSize.computeImageScale(srcSize, decodingInfo.getTargetSize(), decodingInfo
                    .getViewScaleType(), scaleType == ImageScaleType.EXACTLY_STRETCHED);
			if (Float.compare(scale, 1f) != 0) {
				m.setScale(scale, scale);

				if (loggingEnabled) {
					GinULog.d(LOG_SCALE_IMAGE, srcSize, srcSize.scale(scale), scale, decodingInfo.getImageKey());
				}
			}
		}
		// Flip bitmap if need
		if (flipHorizontal) {
			...
			if (loggingEnabled) GinULog.d(LOG_FLIP_IMAGE, decodingInfo.getImageKey());
		}
		// Rotate bitmap if need
		if (rotation != 0) {
			m.postRotate(rotation);

			if (loggingEnabled) GinULog.d(LOG_ROTATE_IMAGE, rotation, decodingInfo.getImageKey());
		}
