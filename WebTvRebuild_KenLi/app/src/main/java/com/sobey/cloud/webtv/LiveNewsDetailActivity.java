...
public class LiveNewsDetailActivity extends BaseActivity implements OnPreparedListener, OnCompletionListener,
	...
	public void onDataFinish(Bundle savedInstanceState) {
		...
		SharedPreferences userInfo = LiveNewsDetailActivity.this.getSharedPreferences("user_info", 0);
	...
	private void initFooter() {
		// View mActivityLayoutView = (RelativeLayout)
		// findViewById(R.id.activity_livenews_detail_layout);
	...
	private void initGuide() {
		mAdapter = new BaseAdapter() {
			...
			public View getView(int position, View convertView, ViewGroup parent) {
				...
				if (convertView == null) {
					convertView = LayoutInflater.from(LiveNewsDetailActivity.this)
	...
	private void replayItemVideo() {
		CheckNetwork checkNetwork = new CheckNetwork(LiveNewsDetailActivity.this);
		if (checkNetwork.check3GOnly(false, null) == CheckNetwork.MOBILE_ONLY) {
			AlertDialog.Builder builder = new AlertDialog.Builder(LiveNewsDetailActivity.this);
	...
	private void initPlayer() {
		...
		// if ("-1".equals(liveMark)) {
		// ImageView mp3View = new ImageView(this);
		// mp3View.setScaleType(ScaleType.FIT_XY);
		// mp3View.setImageResource(R.drawable.listener_online);
		// mp3View.setLayoutParams(oldLp);
		// mViewHolder.addView(mp3View);
		// }
		/**
		 * 注册listener
		 */
						...
						try {
							...
							for (int i = 0; i < guideArray.length(); i++) {
								News.getTvGuide(
										...
										LiveNewsDetailActivity.this, new OnJsonArrayResultListener() {
											@Override
											public void onOK(JSONArray result) {
												if (isDisposed)
													return;
				...
				DateParse.getDate(0, 0, 0, 0, null, null, "yyyyMMdd"), this, new OnJsonArrayResultListener() {
					...
					public void onOK(JSONArray result) {
						...
						try {// 这是去取到一个台的json文件地址 然后再去请求这个json地址 再得到播放地址
							...
							// for (int i = 0; i < guideArray.length(); i++) {
							// News.getTvGuide(((JSONObject)
							// guideArray.get(i)).getString("playerpath"),
							// LiveNewsDetailActivity.this, new
							// OnJsonArrayResultListener() {
							// @Override
							// public void onOK(JSONArray result) {//
							// 这返回的数据是取播放地址的
							// if (isDisposed)
							...
							// //
							// "rtmp://liveh2.ccsobey.com/live/e8e5530ca3e744f1867f208876a84c18"));
							// }
							// mediaObj = new
							// MediaObj(result.optJSONObject(i).optString("C_Name"),
