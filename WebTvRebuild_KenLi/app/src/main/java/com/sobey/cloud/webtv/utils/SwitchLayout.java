	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			// 往左滑动，且不是最后一个
				...
				SharedPreferences newGuide = mContext.getSharedPreferences("newguide", 0);
				if (newGuide == null || TextUtils.isEmpty(newGuide.getString("openFlag", null)) || newGuide.getString("openFlag", "").equalsIgnoreCase("true")) {
					Editor editor = newGuide.edit();
					editor.putString("openFlag", "false");
					editor.commit();
					Intent intent=new Intent(mContext, WebPageFragment.class);
					mContext.startActivity(intent);
//					CatalogControl.startFirstActivity((Activity) mContext);
				}
