...
public class BrokeNewsHomeActivity extends BaseActivity implements IDragListViewListener {
	...
	public int getContentView() {
		...
		return R.layout.activity_broke_news_home;
	}
	...
	public void onWindowFocusChanged(boolean hasFocus) {
		if (hasFocus) {
			...
			if (launchMode != null && launchMode.getBoolean("broke_task_home", false)) {
				startActivity(new Intent(BrokeNewsHomeActivity.this, BrokeTaskHomeActivity.class));
				finish();
			}
		}
	...
	public void init() {
		...
		BrokeFooterControl.setBrokeFooterSelectedItem(this, "最新", mCatalogObj.index);
		setupTabBar();
	...
	private void loadViewHolder(int position, View convertView, ViewHolderBrokeNews viewHolderBrokeNews, int type) {
		...
		try {
			...
			if (TextUtils.isEmpty(imageUrl)) {
				imageUrl = obj.optString("defaultimage");
			}
	...
	private void loadMore(final String catalogId, final int pageIndex) {
		...
		isLoading = true;
		News.getBrokeArticleList(catalogId, null, BrokeStatus.PUBLISHED, mPageSize, (pageIndex == -1 ? 1 : pageIndex),
			...
			public void onOK(JSONArray result) {
				try {
					for (int i = 0; i < result.length(); i++) {
						...
						View view = LayoutInflater.from(BrokeNewsHomeActivity.this)
								.inflate(R.layout.layout_tabitem_text, null);
	...
	private void openDetailActivity(String information) {
		Intent intent = new Intent(BrokeNewsHomeActivity.this, BrokeNewsDetailActivity.class);
		intent.putExtra("information", information);
		BrokeNewsHomeActivity.this.startActivity(intent);
	}
